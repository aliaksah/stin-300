...
This time `x` is the text `"Donald"`, which is the *content* of the first list-element that is an instance of the character class. Please note this right away:
...
Let us look at an example using the `my.list` object from we created above. We want to know if the elements in this list are `character`, i.e. contains a character istance. This is how we could do it:
...
Here is an example that shows us that `Inf` in R is not really infinity(!):
...
We start out by creating the object `x` and give it the value 2. The function `is.finite()` returns `TRUE` as long as the input is some number, and returns `FALSE` if it is `Inf`, which is the **R-symbol for infinite**. Inside the loop we print `x`, and then we square it. The latter is crucial. In the logical expression of the `while`-loop we make some test/comparison on `x`. Then this object *must change* inside the loop, otherwise this test/comparison will turn out the same forever. Mathematically this loop should run forever, since doubling will never take us to infinitiy, but fortunately, computers can never handle very big numbers, and we reach `Inf`after some iterations.
...
-   the value to assign if `TRUE`

-   and the value to assign if `FALSE`.

Here is a simple example:

```{r}
library(stringr)                       # need str_length()
...
Do the same for the **list.1** data structure in the second list. Use Sys.time to record the run-time of the two loops. Does it take longer with lists?
...
We want to count the number of words in each line. Anything separated by a space (`" "`) is a word. Use `str_split()` to split all lines into words. This produces a list. Loop through the list, count the number of words, and store this in a vector with one element for each list-element.
...
Make a `while`-loop that keeps on rolling dice until all dice have been collected. Also, keep track of how many rolls were made (repetitions of the `while`-loop). This is best done by having a integer object that increases with 1 in each repetition.
